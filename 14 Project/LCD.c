...
void LCD_PIN_Init(void){
	...
	GPIOB->MODER &= ~(0xFF0C0F0F); //MODER Mask
	GPIOB->MODER |= 0xAA080A0A; //MODER Value
	...
	// Configure Port D Pins 8-15 as Alternate Function Mode
	GPIOD->MODER &= ~(0xFFFF0000);
	GPIOD->MODER |= 0xAAAA0000;

	// Configure Port D Pins 8-15 as Alternate Function 11
...
void LCD_Configure(void){
  ...
  //Configure DUTY[2:0] bits of LCD_CR and set the duty to 1/4 (011)
	...
	//Configure CC[2:0] bits of LCD_FCR and set the contrast value to the maximum (111)
	LCD->FCR &= ~(LCD_FCR_CC);
	LCD->FCR |= LCD_FCR_CC_0;
	LCD->FCR |= LCD_FCR_CC_1;
	LCD->FCR |= LCD_FCR_CC_2; //3.51V

	//Configure PON[2:0] bits of LCD_FCR and set the pulse on period to the maximum (111)
	LCD->FCR &= ~(LCD_FCR_PON);
...
void LCD_DisplayString(uint8_t* ptr){
	...
	for (i = 0; i < 6; i++) {
			if (i == 1) {
			LCD_WriteChar(ptr+i,0, 1, i);
			}else if (i == 3) {
				LCD_WriteChar(ptr+i,1, 0, i);
			}else {
...
void Time_Conversion(uint32_t milliseconds, uint8_t* display) {
	uint8_t time[6];
	uint32_t min1, min2, secs1, secs2, mili1, mili2, timeLeft;

	min1 = milliseconds / 60000;
	timeLeft = milliseconds - (60000 * min1);
